using Microsoft.EntityFrameworkCore;
using System;
using System.Linq;
using System.Runtime.CompilerServices;

[assembly: InternalsVisibleTo(@"dn32.infra.EntityFramework.SqlServer, PublicKey=")]
[assembly: InternalsVisibleTo(@"dn32.infra.EntityFramework.PostgreSQL, PublicKey=")]
[assembly: InternalsVisibleTo(@"dn32.infra.EntityFramework.MySQL, PublicKey=")]
namespace dn32.infra
{
    /// <summary>
    /// Obtetos de transação.
    /// </summary>
    public class EFTransactionObjects : IDnObjetosTransacionais
    {
        public IDnDbContext contexto { get; set; }

        public DnDbContext DnDbContext
        {
            get => contexto as DnDbContext;
            set => contexto = value;
        }

        public SessaoDeRequisicaoDoUsuario UserSessionRequest { get; set; }

        public void Dispose()
        {
            this.DnDbContext.Dispose();
        }

        /// <summary>
        /// Initializes a new instance of the <see cref="EFTransactionObjects"/> class. 
        /// Inicializa objeto de transação.
        /// </summary>
        /// <param Nome="dataBaseConnectionString">
        /// String de conexão com o banco de dados.
        /// </param>
        public EFTransactionObjects(Conexao connection, SessaoDeRequisicaoDoUsuario userSessionRequest)
        {
            this.UserSessionRequest = userSessionRequest;
            this.DnDbContext = ContextFactory.Create(connection, UserSessionRequest);
        }

        public IQueryable<TX> ObterObjetoInputInterno<TX>() where TX : DnEntidadeBase
        {
            return this.DnDbContext.Set<TX>();
        }

        public IQueryable ObterObjetoInputDataInterno(Type type)
        {
            return typeof(DbContext).GetMethod(nameof(DbContext.Set))?.MakeGenericMethod(type).Invoke(DnDbContext, null).DnCast<IQueryable>();
        }

        /// <summary>
        /// Obtem o referência de uma tabela do banco de dados.
        /// </summary>
        /// <typeparam Nome="TX">
        /// Tipo de entidade desejada.
        /// </typeparam>
        /// <returns>
        /// A referência da tabela do banco de dados.
        /// </returns>
        public virtual IQueryable<TX> ObterObjetoQueryInterno<TX>() where TX : DnEntidadeBase
        {
            return this.DnDbContext.Set<TX>();
        }
    }
}