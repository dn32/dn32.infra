...
namespace dn32.infra
{
    internal class DnDescricaoDaClasse
    {
        ...
        private static void AdicionarPropriedadeDeComposicao(List<DnDescricaoDePropriedade> propriedades, Type tipoPrincipal, string propriedade)
        {
            ...
            var propertyInfo = tipoPrincipal.GetProperty(nomeDaClasse) ??
                throw new InvalidOperationException($"A entidade '{tipoPrincipal.Name}' não possui uma propriedade com nome '{nomeDaClasse}'");
        ...
        {
            var propertyInfo = tipoPrincipal.GetProperties().FirstOrDefault(x => x.Name.Equals(nomeDaPropriedade, StringComparison.InvariantCultureIgnoreCase)) ??
                throw new InvalidOperationException($"A entidade '{tipoPrincipal.Name}' não possui uma propriedade com nome '{nomeDaPropriedade}'");
