...
namespace dn32.infra.Teste.Extensoes
{
    ...
    {
        [Test]
        public void SerializarParaFluenteJsonSerializacaoSimples()
        {
            const int obj = 1;
            const string json = "1";
            var jsonRetornado = obj.SerializarParaFluenteJson();

            Assert.AreEqual(json, jsonRetornado);
        }

        [Test]
        public void SerializarParaFluenteJsonConfiguracoesDeSerializacaoMinusculo()
        {
            var obj = new { Valor = 1 };
